...
logging.basicConfig(filename='scrape_test.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
...
def batch_scrape(urls, prompt, config):
    ...
    for url in urls:
        try:
            ...
            has_profile = isinstance(profile, dict) and all(key in profile for key in ['About Us (including locations)', 'Our Culture', 'Our Team', 'Noteworthy & Differentiated'])
            has_media = isinstance(media, list) and len(media) > 0
            media_valid = all('url' in m and 'type' in m for m in media) if has_media else True
            error = None
            if not has_profile:
                error = 'Incomplete profile'
            if not media_valid:
                error = (error + ' ' if error else '') + 'Invalid media format'
            logging.info(f"URL: {url} - Profile OK: {has_profile} - Media OK: {has_media and media_valid} - Error: {error}")
            results.append({'url': url, 'result': result, 'error': error})
        except Exception as e:
            logging.error(f"URL: {url} - Error: {str(e)}")
            results.append({'url': url, 'result': {'profile': {}, 'media': []}, 'error': str(e)})  # Graceful failure with empty result
    return results
...
def test_prompts(urls, prompts, config, batch_size=20):
    ...
    for prompt_name, prompt in prompts.items():
        ...
        avg_media = sum(len(r['result'].get('media', [])) for r in results if r['result']) / len(results)
        ...
        logging.info(f"Tested {prompt_name} - Success Rate: {success_count / batch_size:.2f} - Avg Media: {avg_media:.2f}")
    return comparisons
...
if __name__ == "__main__":
    ...
    prompts = {
        'Prompt1': """Output ONLY valid JSON. Do not include any additional text, explanations, or wrappers. Ensure the JSON is parseable without errors. """ + default_prompt,
        'Prompt2': default_prompt + """ Example output: {'profile': {'About Us (including locations)': 'Description...', 'Our Culture': 'Description...', 'Our Team': 'Description...', 'Noteworthy & Differentiated': 'Description...'}, 'media': [{'url': 'http://example.com/logo.png', 'type': 'image'}]}. Follow this structure exactly.""",
        'Prompt3': default_prompt + """ If a section or media is unavailable, use 'Not available' for strings or empty array for media. Ensure all four profile keys are always present."""
    }
