...
class TestSplitAPI:
    ...
    def test_scrape_text_endpoint(self, mock_find_about, mock_extract_company, mock_get_content, client):
        ...
        mock_about_pages = [
            {"url": "https://test.com/about", "title": "About Us", "relevance_score": 0.9}
        ]
    ...
    def test_scrape_media_endpoint_no_cursor(self, mock_extract_media, mock_get_content, client):
    ...
    @patch('about_us_scraper_service.api.main_split.get_page_content')
    @patch('about_us_scraper_service.api.main_split.extract_media_assets')
    def test_scrape_media_endpoint_with_cursor(self, mock_extract_media, mock_get_content, client):
    ...
    @patch('about_us_scraper_service.api.main_split.get_page_content')
    @patch('about_us_scraper_service.api.main_split.extract_company_info_programmatic')
    def test_scrape_enhance_endpoint(self, mock_extract_company, mock_get_content, client):
    ...
    def test_scrape_text_endpoint_invalid_url(self, client):
        ...
        with patch('about_us_scraper_service.api.main_split.get_page_content') as mock_get_content:
    ...
    def test_scrape_media_endpoint_invalid_cursor(self, client):
        ...
        with patch('about_us_scraper_service.api.main_split.get_page_content') as mock_get_content:
            ...
            with patch('about_us_scraper_service.api.main_split.extract_media_assets') as mock_extract:
                mock_extract.return_value = {
                    "media_assets": [],
                    "media_summary": {"total_assets": 0, "images_count": 0, "videos_count": 0, "documents_count": 0, "icons_count": 0, "current_page_count": 0, "has_more": False},
                    "pagination": {"next_cursor": None, "has_more": False, "total_count": 0, "current_page_start": 0, "current_page_end": 0}
                }
                
                # Test with invalid cursor (should default to start_index = 0)
                response = client.get("/scrape/media?url=test.com&cursor=invalid-cursor&limit=10")
    ...
    def test_legacy_endpoints_redirect(self, client):
        ...
        with patch('about_us_scraper_service.api.main_split.get_page_content') as mock_get_content:
            ...
            with patch('about_us_scraper_service.api.main_split.extract_company_info_programmatic') as mock_extract:
                mock_extract.return_value = {
                    "title": "Test",
                    "description": "Test",
                    "content": "Test",
                ...
                with patch('about_us_scraper_service.api.main_split.find_about_pages') as mock_find:
...
class TestCursorPagination:
    ...
    def test_cursor_pagination_continuity(self):
        ...
        mock_soup = Mock()
        mock_img1 = Mock()
        mock_img1.get.side_effect = lambda x: "https://test.com/img1.png" if x == "src" else ""
        mock_img1.name = "img"
        
        mock_img2 = Mock()
        mock_img2.get.side_effect = lambda x: "https://test.com/img2.png" if x == "src" else ""
        mock_img2.name = "img"
        
        mock_soup.find_all.side_effect = lambda x: [mock_img1, mock_img2] if x == "img" else []
        ...
        if result1["pagination"]["has_more"]:
            ...
            # Verify continuity
            assert result2["pagination"]["current_page_start"] == result1["pagination"]["current_page_end"]
            assert result2["pagination"]["total_count"] == result1["pagination"]["total_count"]
