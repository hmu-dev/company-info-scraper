...
def get_video_metadata(url):
    ...
    try:
        ...
        return {
            ...
            "bitrate": (
                int(video_info.get("bit_rate", 0)) if "bit_rate" in video_info else 0
            ),
        }
    except Exception as e:
...
st.caption(
    "This app allows you to scrape a website using OpenAI API and display media content"
)
...
def download_media_to_base64(media_url, base_url, context="", session=None):
    ...
    try:
        ...
        if media_url.startswith("data:"):
            ...
            if "viewBox=" in media_url and (
                "0 0" in media_url or "placeholder" in media_url.lower()
            ):
                ...
                try:
                    ...
                    if "svg" in content_type:
                        ...
                        svg_path = os.path.join(
                            "scraped_media", f"temp_{hash(media_url) % 10000}.svg"
                        )
        ...
        if response.status_code == 200:
            ...
            if content_length > 10 * 1024 * 1024:  # Skip files larger than 10MB
                logger.warning(
                    f"Skipping large file {media_url} ({content_length / 1024 / 1024:.1f}MB)"
                )
                return None, None, None, None, None, 0, context
            ...
            if f".{file_ext}" in ALLOWED_VIDEO_FORMATS:
                ...
                if duration is None:
                    logger.warning(
                        f"Could not determine video duration for {media_url}"
                    )
                    return None, None, None, None, None, 0, context

                if duration > MAX_VIDEO_DURATION_SECONDS:
                    logger.warning(
                        f"Video too long ({duration:.1f} seconds) - skipping: {media_url}"
                    )
                    return None, None, None, None, None, 0, context
                ...
                estimated_size_mb = (metadata["bitrate"] * duration) / (
                    8 * 1024 * 1024
                )  # Convert bits to MB
                if estimated_size_mb > MAX_VIDEO_SIZE_MB:
                    logger.warning(
                        f"Video too large (estimated {estimated_size_mb:.1f}MB) - skipping: {media_url}"
                    )
                    return None, None, None, None, None, 0, context

                logger.info(
                    f"Video accepted: {media_url} (duration: {duration:.1f}s, size: {estimated_size_mb:.1f}MB)"
                )
                    ...
                    if file_ext.lower() == "svg" or metadata["format"].lower() == "svg":
                        try:
                            png_path = os.path.join(
                                "scraped_media", f"{os.path.splitext(file_name)[0]}.png"
                            )
            ...
            for img in img_tags:
                ...
                if src:
                    ...
                    if any(
                        ...
                        media_urls.append(
                            (src, "image", f"Image: {alt_text or 'Company image'}")
                        )
                    # Also include images that seem to be content images (not UI elements)
                        ...
                        media_urls.append(
                            (src, "image", f"Image: {alt_text or 'Content image'}")
                        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for link in all_links:
                ...
                if any(keyword in href for keyword in about_keywords) or any(
                    phrase in link_text
                    for phrase in [
                        "about us",
                        "about",
        ...
        if isinstance(content, dict):
            for key, value in content.items():
                ...
                if key.lower() in [
                    ...
                    local_path, original_url, media_type = download_media(
                        value, base_url
                    )
                    if local_path:
                        downloaded_media.append(
                            (local_path, original_url, current_path, media_type)
                        )
            ...
            if any(ext in content.lower() for ext in media_extensions):
                ...
                if local_path:
                    downloaded_media.append(
                        (local_path, original_url, path, media_type)
                    )
...
if openai_access_token:
    ...
    auto_find_about = st.checkbox(
        "Automatically find About Us page if not provided",
        value=True,
    ...
    if scrape_mode in ["Combined (Profile + Media)", "Media Only"]:
        include_media = st.checkbox(
            "Download and display media files",
            value=True,
...
Keep descriptions professional, positive, and suitable for a mobile app experience. Extract relevant media (logos, company photos, videos) for branding. Response must be valid JSON with 'profile' object (containing all five sections) and 'media' array (objects with 'url' and 'type')."""
    ...
    include_media = st.checkbox(
        "Download and display media files (images & videos)", value=True
    )
            ...
            try:
                ...
                if auto_find_about and scrape_mode != "Media Only":
                    with st.spinner("Looking for About Us page..."):
                        ...
                        if found_about_url != url:
                            urls_to_try.insert(
                                0, found_about_url
                            )  # Try the found About page first
                ...
                if scrape_mode in ["Combined (Profile + Media)", "Profile Only"]:
                    for try_url in urls_to_try:
                        try:
                            with st.spinner(
                                f"Extracting company profile from {try_url}..."
                            ):
                                ...
                                if result and isinstance(result, dict):
                                    ...
                                    if content and any(content.values()):
                                        ...
                                        location = (
                                            "2750 N. Digital Drive, Lehi, UT 84043"
                                        )
                                        ...
                                        for pattern in location_patterns:
                                            match = re.search(
                                                pattern, about_us, re.IGNORECASE
                                            )
                                            if match:
                                        ...
                                        if try_url != url:
                                            st.success(
                                                f"✅ Successfully extracted profile from: {try_url}"
                                            )
                                        break

                        except Exception as e:
                            st.warning(
                                f"⚠️ Error extracting profile from {try_url}: {str(e)}"
                            )
                            continue
                        ...
                        for media_url, media_type, context in html_media:
                            if media_url not in seen_urls:
                                ...
                                (
                                    ...
                                    priority,
                                    ctx,
                                ) = download_media_to_base64(
                                    media_url, scrape_url, context
                                )
                ...
                if scrape_mode == "Media Only":
                    ...
                    result = {
                        ...
                        "media": (
                            [] if scrape_mode == "Combined (Profile + Media)" else None
                        ),
                    }
                    ...
                    with st.spinner("Extracting media from page..."):
                        ...
                        for media_url, media_type, context in html_media:
                            if media_url not in seen_urls:
                                ...
                                if url:
                                    downloaded_media.append(
                                        (url, type_, ctx, filename, metadata, priority)
                                    )

                    if downloaded_media:
                        # Separate images and videos
                        images = [
                            item for item in downloaded_media if item[1] == "image"
                        ]
                        videos = [
                            item for item in downloaded_media if item[1] == "video"
                        ]

                        st.success(
                            f"Found and downloaded {len(images)} image(s) and {len(videos)} video(s)"
                        )
                                ...
                                with cols[i % 3]:
                                    try:
                                        ...
                                        image_path = os.path.join(
                                            "scraped_media", display_filename
                                        )
                                        if not os.path.exists(image_path):
                                            st.warning(
                                                f"Image file not found: {display_filename}"
                                            )
                                            continue

                                        log_state(
                                            f"Attempting to display image: {image_path}"
                                        )
                                        try:
                                            ...
                                            if (
                                                ...
                                                log_state(
                                                    f"Skipping large image {image_path} ({file_size / 1024 / 1024:.1f}MB)"
                                                )
                                                continue
                                            ...
                                            with open(image_path, "rb") as img_file:
                                                ...
                                                log_state(
                                                    f"Successfully read {len(image_bytes)} bytes from {image_path}"
                                                )

                                            # Display using bytes directly
                                            st.image(
                                                image_bytes,
                                                caption=f"Image from: {context}",
                                                width="stretch",
                                            )
                                            ...
                                            log_exception(
                                                e,
                                                f"Memory error processing {image_path}",
                                            )
                                            continue
                                            log_state(
                                            ...
                                            log_exception(
                                                e,
                                                f"Failed to display image: {image_path}",
                                            )
                                            st.error(
                                        ...
                                        st.error(
                                            f"Error displaying image {filename}: {str(e)}"
                                        )
                        ...
                        if videos:
                            ...
                            for (
                                ...
                                try:
                                    ...
                                    st.error(
                                        f"Error displaying video {local_path}: {str(e)}"
                                    )
