...
async def test_download_media_from_cache(media_service):
    ...
    with patch('requests.get', return_value=mock_response), \
         patch.object(media_service, 'process_image', return_value=(b'processed_data', {'width': 100, 'height': 100, 'format': 'jpeg', 'size_bytes': 1024})):
        asset = await media_service.download_media("https://example.com/image.jpg")
        assert asset.type == "image"
        assert str(asset.url) == "https://example.com/image.jpg"
        assert asset.metadata.width == 100
...
async def test_download_media_http_error(media_service):
    ...
    with patch('requests.get', return_value=mock_response):
        with pytest.raises(MediaProcessingError, match="Download failed with status 404"):
...
async def test_process_image_svg(media_service):
    ...
    with patch('cairosvg.svg2png', return_value=png_data):
        content, metadata = await media_service.process_image(svg_content, 'image/svg+xml')
...
async def test_process_video_success(media_service, sample_video):
    ...
    with patch('ffmpeg.probe', return_value=sample_video), \
         patch('builtins.open', create=True), \
         patch('os.path.exists', return_value=True), \
         patch('os.remove'):
        content, metadata = await media_service.process_video(video_content, 'video/mp4')
...
async def test_process_video_duration_limit(media_service):
    ...
    with patch('ffmpeg.probe', return_value=long_video), \
         patch('builtins.open', create=True), \
         patch('os.path.exists', return_value=True), \
         patch('os.remove'):
...
async def test_process_video_probe_error(media_service):
    ...
    with patch('ffmpeg.probe', side_effect=ffmpeg.Error("Probe failed", stdout=b"", stderr=b"Error probing video")), \
         patch('builtins.open', create=True), \
         patch('os.path.exists', return_value=True), \
         patch('os.remove'):
...
async def test_store_media_success(media_service):
    ...
    url = await media_service.store_media(media_data, 'image/jpeg', 'https://example.com/image.jpg')
...
async def test_store_media_error(media_service):
    ...
    with pytest.raises(MediaProcessingError, match="Failed to store media"):
        await media_service.store_media(b'test_data', 'image/jpeg', 'https://example.com/image.jpg')
