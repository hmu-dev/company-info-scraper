...
def app():
    ...
    limiter = RateLimiter(
        requests_per_minute=5, burst_limit=5, key_func=lambda r: r.client.host
    )

    app.add_middleware(RateLimitMiddleware, limiter=limiter, exclude_paths=["/health"])
...
def test_rate_limiter_init():
    ...
    limiter = RateLimiter(
        requests_per_minute=60, burst_limit=10, key_func=lambda r: r.client.host
    )
...
def test_create_rate_limiter():
    ...
    limiter = create_rate_limiter(
        requests_per_minute=30, burst_limit=5, key_func=lambda r: r.client.host
    )
...
def test_rate_limit_custom_key_func():
    ...
    limiter = RateLimiter(
        requests_per_minute=60, burst_limit=10, key_func=custom_key_func
    )
...
def test_rate_limit_concurrent_requests(client):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(client.get, "/test") for _ in range(10)]
