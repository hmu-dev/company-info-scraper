...
def mock_storage():
    with patch("api.endpoints.media.MediaStorage") as mock:
        ...
        storage_instance.store_media = AsyncMock(return_value=("https://cdn.example.com/logo.png", {
            "width": 200,
            "height": 200,
            "size_bytes": 15000,
            "format": "png",
...
def test_scrape_media_success(client, mock_storage, mock_extract_media, mock_paginate):
    ...
    assert data["media"][0]["metadata"]["priority"] == 95  # Updated to match new priority calculation
...
def test_scrape_media_storage_error(client, mock_storage, mock_extract_media, mock_paginate):
    # Setup
    mock_storage.store_media = AsyncMock(side_effect=StorageError("Failed to store media"))
