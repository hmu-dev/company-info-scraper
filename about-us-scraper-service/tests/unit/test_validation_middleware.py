...
def app():
    ...
    app.add_middleware(
        ValidationMiddleware, validator=validator, exclude_paths=["/health"]
    )
...
def test_validate_request_body_invalid_api_key(validator):
    ...
    body = {"url": "https://example.com", "openai_api_key": "invalid-key"}
    error = validator.validate_request_body(body)
    assert error == "Invalid API key format"
...
def test_validate_request_body_invalid_model(validator):
    ...
    body = {"url": "https://example.com", "model": "invalid!model"}
    error = validator.validate_request_body(body)
...
def test_middleware_content_length_too_large(client):
    ...
    response = client.post(
        "/test", headers=headers, json={"url": "https://example.com"}
    )
...
def test_middleware_invalid_json(client):
    ...
    response = client.post(
        "/test", headers={"content-type": "application/json"}, content="invalid json"
    )
...
def test_setup_validation():
    ...
    response = client.post("/test", json={"url": "https://blocked.com"})
    assert response.status_code == 400
    ...
    response = client.post(
        "/test", headers={"content-length": "3000"}, json={"url": "https://example.com"}
    )
    assert response.status_code == 413
    assert "Content length exceeds maximum" in response.json()["error"]
