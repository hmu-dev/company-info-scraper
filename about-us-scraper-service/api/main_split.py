...
def find_about_pages(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
    ...
    for link in soup.find_all('a', href=True):
        ...
        if href:
            ...
            if any(keyword in link_text for keyword in ['about', 'company', 'team', 'story', 'mission']):
                about_pages.append({
                    "url": full_url,
    ...
    return sorted(unique_pages.values(), key=lambda x: x["relevance_score"], reverse=True)

def extract_media_assets(soup: BeautifulSoup, base_url: str, cursor: str = None, limit: int = 20) -> Dict[str, Any]:
    ...
    for img in soup.find_all('img'):
        ...
        if src:
            ...
            elif not any(ui_element in full_url.lower() for ui_element in ['icon', 'button', 'arrow', 'cart']):
        ...
        if href:
            ...
            if file_ext in ['pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx']:
                all_media.append({
                    ...
                    "priority": 70,
                    "context": link.get_text().strip() or f"Document ({file_ext.upper()})"
                })
...
async def scrape_media_paginated(
    ...
    limit: int = Query(20, description="Number of media assets per page (max 50)", le=50),
    media_type: Optional[str] = Query(None, description="Filter by media type: image, video, document, icon")
):
    """
    ...
    try:
        ...
        if media_type:
            media_data["media_assets"] = [
                asset for asset in media_data["media_assets"] 
...
async def enhance_with_ai(
    url: str,
    text_data: Optional[str] = Query(None, description="Pre-extracted text data to enhance")
):
    """
    🧠 **AI ENHANCEMENT**
