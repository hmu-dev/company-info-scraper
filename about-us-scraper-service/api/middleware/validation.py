...
class RequestValidator:
    ...
    def validate_api_key(self, api_key: Optional[str]) -> Optional[str]:
        ...
        # Check length (only if format is valid)
        if len(api_key) < 32:
...
class ValidationMiddleware:
    def __init__(self, app, validator: RequestValidator, exclude_paths: List[str] = None):
...
def setup_validation(
    ...
    app.add_middleware(
        ...
        validator=validator,
        exclude_paths=exclude_paths or ["/health", "/metrics", "/openapi.json"]
    )
