            ...
            media_assets["images"].append(
                {
                    ...
                    "priority": (
                        ...
                        if any(
                            ...
                            for keyword in ["logo", "brand", "company"]
                        )
                        else "medium"
                    ),
                }
    ...
    for nav in nav_links:
        for link in nav.find_all("a", href=True):
            ...
            for pattern in about_patterns:
                if re.search(pattern, href) or any(
                    keyword in text for keyword in ["about", "company", "team", "story"]
                ):
                    ...
                    about_pages.append(
                        {
                            ...
                            "confidence": (
                                "high"
                                if any(
                                    keyword in text for keyword in ["about", "company"]
                                )
                                else "medium"
                            ),
                        }
...
async def scrape_intelligent(
    url: str,
    force_ai: bool = Query(
        False, description="Force AI analysis even if programmatic results are good"
    ),
    max_about_pages: int = Query(
        3, description="Maximum number of about pages to analyze"
    ),
    ...
    **🎯 Perfect for comprehensive company analysis!**
    ...
    try:
        ...
        for about_page in about_pages[:max_about_pages]:
            try:
                about_soup, about_html, about_status = get_page_content(
                    about_page["url"]
                )
                ...
                about_page_analysis.append(
                    {
                        "url": about_page["url"],
                        "title": (
                            about_soup.find("title").get_text().strip()
                            if about_soup.find("title")
                            else ""
                        ),
        ...
        if use_ai:
            ai_enhancement = {
                "used": True,
                "reason": (
                    "Low confidence programmatic results"
                    if company_info["confidence"] == "low"
                    else "Forced by parameter"
                ),
        ...
        return {
            ...
            "content": (
                all_about_content[:3000] + "..."
                if len(all_about_content) > 3000
                else all_about_content
            ),
