...
class MediaService:
    def __init__(self,
        ...
        self.allowed_video_types = supported_video_types or [
    ...
    async def download_media(self, url: str) -> MediaAsset:
        ...
        try:
            ...
            if not any(t in content_type for t in 
                     self.allowed_image_types + self.allowed_video_types):
                raise MediaProcessingError(
            ...
            media_type = "image" if any(t in content_type for t in self.allowed_image_types) else "video"
    ...
    async def process_image(self, 
                          content: bytes,
                          content_type: str) -> Tuple[bytes, Dict[str, Any]]:
        """
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise MediaProcessingError(f"Failed to extract video duration: {str(e)}")
        ...
        try:
            ...
            if cached_data:
                ...
                return MediaAsset(
                    ...
                    context=data.get("context", f"{data['type'].title()} from {data['url']}")
                )
            return None
        except Exception as e:
    ...
    async def update_cache(self, url: str, media_asset: MediaAsset, ttl_seconds: int = 3600) -> None:
        """
