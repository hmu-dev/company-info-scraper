...
def log_event(event_type: str, data: Dict[str, Any], request_id: Optional[str] = None) -> None:
    ...
    def wrapper(event, context):
        ...
        try:
            ...
            if isinstance(result, dict) and "token_usage" in result:
                metrics.update({
                    "TokensUsed": (result["token_usage"], "Count")
                })
...
def log_llm_request(llm_data: Dict[str, Any], request_id: Optional[str] = None) -> None:
    ...
    metrics = {
        ...
        "TotalTokens": (llm_data["prompt_tokens"] + llm_data["completion_tokens"], "Count")
    }
    publish_metrics("AiWebScraper", metrics)

def log_media_metrics(media_data: Dict[str, Any], request_id: Optional[str] = None) -> None:
    """Log media processing metrics"""
    log_event("media_processing", {
        "url": media_data["url"],
...
def log_cache_metrics(cache_data: Dict[str, Any], request_id: Optional[str] = None) -> None:
    """Log cache interaction metrics"""
    log_event("cache_interaction", {
